# IIG：Claude Code 循环优化 Gate

[English](README.md) | 中文

IIG 是一个用于循环优化工作的强制 gate Agent Skill：先激活 gate，再计划，隔离高风险改动，用真实命令验证，记录结果，谨慎提交，合并，清理，然后再决定是否继续下一轮。

仓库名和 skill 命令仍然是 `iterative-improve`。IIG 不绑定具体仓库，适合重构、迁移、策略实验、报告生成流程、数据工作流、质量改进循环，以及任何不希望 AI 编程代理一路跑偏的任务。
